define('tui/storage', ['tui/class'], function(Class) {
	var Storage = Class({
		initialize : function(src) {
			...
			if(self.iframe1.on) {
				self.iframe1.on('load', onload);
			}
			else {
				self.iframe1.bind('load', onload);
			}
			function onload() {
	...
	function init(win, origin) {
		...
		//ff13
		if(win !== window && $.browser.mozilla && (version >= 13 && version <= 14 || version >= 30))
	...
	function initInLowIe(win) {
		...
		storage.setItem = function(n, v, cb){
			try {
				...
				throw new Error('[V3] userData behavior error');
			}
			if(cb)
				cb();
		};
