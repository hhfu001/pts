define('tui/view', ['tui/event'], function(Event){
	...
	var viewInstance = {};
	// 默认视图属性
	var viewDefaultOptions = ['el', 'id', 'attrs', 'className', 'tagName', 'model', 'preventHighlight'];
	// 事件代理正则
	...
	$.extend(View.prototype, {
		...
		undelegateEvents: function(eventName, selector) {
			...
			if (this.options.preventHighlight) {
				this.$el[0].ontouchend = null;
			}
			return this;
		},
		...
		_ensureTouch: function(eventName, selector){
			if (isTouch && window.tudouClickStat) {
				...
				if ((eventName == 'click' || eventName == 'tap' || eventName == 'singleTap') && selector !== '') {
					var ckName = 'click.delegateEvents' + this.clsid;
					...
					// 阻止定制android点击后显示选中框
					if (this.options.preventHighlight) {
						this.$el[0].ontouchend = function(event){
							event.preventDefault();
						};
					}
				}
