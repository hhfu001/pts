...
;(function($){
  function detect(ua){
    ...
    os.phone  = !!(!os.tablet && (android || iphone ||
  ...
  $(document).ready(function(){
    ...
    $(document)
      .on(touchStartEvent, function(e){
        ...
        touch.x1 = _x = firstTouch.pageX
        touch.y1 = _y = firstTouch.pageY
      ...
      .on(touchMoveEvent, function(e){
        ...
        firstTouch = _isPointerType ? e : e.touches[0]
        cancelLongTap()
        touch.x2 = _x = firstTouch.pageX
        touch.y2 = _y = firstTouch.pageY

        deltaX += Math.abs(touch.x1 - touch.x2)
        deltaY += Math.abs(touch.y1 - touch.y2)
      })
      .on(touchEndEvent, function(e){
        if (_isPointerType && !isPrimaryTouch(e)) return
        cancelLongTap()
    ...
    window.addEventListener('click', function (e) {
        var time_threshold = 1000, space_threshold = 30;
        if (new Date() - _t <= time_threshold &&
            Math.abs(e.pageX - _x) <= space_threshold &&
            Math.abs(e.pageY - _y) <= space_threshold) {
            e.stopPropagation();
            e.preventDefault();
        }
    }, true)
